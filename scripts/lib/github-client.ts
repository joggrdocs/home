 ...
 */
export async function createGitHubClient(): Promise<Result<GitHubClient>> {
  const [queryError, queries] = await loadQueries()
  if (queryError) {
    return [
  ...
  const client: GitHubClient = {
    issues: {
      ...
      get: async (issueNumber) => {
        ...
        try {
          ...
          return [
            null,
            { number: data.number, title: data.title, body: data.body ?? '', url: data.url },
          ]
        } catch {
    ...
    projects: {
      ...
      fields: {
        ...
        updateOptions: async ({ fieldId, options }) => {
          ...
          const [execError] = await gh(['api', 'graphql', '--input', '-'], JSON.stringify(mutation))
      ...
      items: {
         ...
         */
        list: async ({ owner, number, limit = 1000 }) => {
          const [execError, stdout] = await gh([
            'project',
            'item-list',
            String(number),
          ...
          try {
            const data = JSON.parse(stdout) as { items: ProjectItem[] }
        ...
        /**
         * Adds an item to a project with optional status.
         */
        add: async ({ owner, number, projectId, issueUrl, statusFieldId, statusOptionId }) => {
    ...
    graphql: async ({ query, variables }) => {
      ...
      const [execError, stdout] = await gh(['api', 'graphql', '--input', '-'], JSON.stringify(body))
